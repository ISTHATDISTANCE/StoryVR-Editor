...
export async function generateEnvironmentImageWithCodex({
  ...
  try {
    ...
    const generationPrompt = buildCodexEnvironmentGenerationPrompt(sceneDescription, referenceImagePaths, {
    ...
    diagnostics.prompt = generationPrompt;
...
export async function generateMatchingGroundTextureWithCodex({
  ...
  try {
    ...
    const generatedImageSnapshot = await snapshotGeneratedPngs(generatedImagesRoot);
    const generationStartedAt = Date.now();
    const generationPrompt = buildCodexMatchingGroundPrompt(sceneDescription, referencePath, {
